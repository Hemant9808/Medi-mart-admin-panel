...
export default function CreateProduct({
  ...
  const [allCategories, setAllCategories] = useState<string[]>([]); // All categories including new ones
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]); // Selected categories
  const [newCategory, setNewCategory] = useState<string>(""); // Input for new category

  // Load existing categories from the backend (mocked here for demo)
  ...
  const handleAddCategory = () => {
    if (newCategory && !allCategories.includes(newCategory)) {
      ...
      setNewCategory(""); // Reset input field after adding
    }
  };

  // Handle selecting/unselecting a category with checkbox
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    const response = await fetch("https://medimart-nayg.onrender.com/product/addProducts", {
      ...
      body: JSON.stringify({
        ...data,
        images: imagesArray,
        categories: selectedCategories, // Assuming categories are passed as an array
      }),
    });
    ...
    <>
      {visible && (
        <div className=" inset-0 z-50 h-[100%]  fixed flex items-center justify-center bg-black bg-opacity-75">
          <div className="bg-white overflow-scroll h-[90%] rounded-lg p-8 shadow-lg max-w-md w-full">
